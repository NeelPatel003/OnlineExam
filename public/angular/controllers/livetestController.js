myApp.controller("livetestController", [
  "$http",
  "$timeout",
  "$scope",
  "$stateParams",
  ...
  function (
    ...
    this.currentPage = 0;
    this.pageSize = 1;
    this.numberOfPages = Math.ceil(live.questions / live.pageSize);
    ...
    this.getLiveQuestions = () => {
      live.questions = [];
      apiService.viewQuestions(live.testid).then(
    ...
    this.submitAnswers = () => {
      ...
      apiService.submitTest(performanceInfo).then(
        function successCallBack(response) {
    ...
    this.calculatePerformance = () => {
      ...
      for (i = 0; i < live.questions.length; i++) {
        for (j = 0; j < live.useroptions.length; j++) {
          if (
            typeof live.useroptions[j] !== "undefined" &&
            live.useroptions[j] !== null
          ) {
      ...
      if (live.useroptions.length < live.questions.length) {
        ...
        while (i--) {
          if (typeof live.useroptions[i] === "undefined") live.skipped++;
        }
      }
